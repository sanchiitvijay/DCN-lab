...
main(int argc, char* argv[])
{
    ...
    std::string transportProt = "Udp"; // Change to Udp
...
// Comment the below lines
    ...
    QueueDiscContainer qdiscs = tch.Install(devices12);
    ...
    Ptr<NetDevice> nd = devices12.Get(1);
    ...
    */
    ...
    // Make interfaces -> interfaces01 And devices -> devices01
    address01.SetBase("10.1.1.0", "255.255.255.0");
    Ipv4InterfaceContainer interfaces01 = address01.Assign(devices01);
    
    // Copy above two lines and Change Source IP address and devices and interfaces
    ...
    ApplicationContainer sinkApp = packetSinkHelper.Install(nodes.Get(2)); // Chage to your last node (2) 
    ...
    InetSocketAddress rmt(interfaces12.GetAddress(1), port); // Changet interfaces -> interfaces12.GetAdress(1) if n nodes then make it n-1
    rmt.SetTos(0xb8);
    AddressValue remoteAddress(rmt);
    onoff.SetAttribute("Remote", remoteAddress);
    apps.Add(onoff.Install(nodes.Get(0))); // Change Get(1) -> Get(0)
